# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 00:20:51 2020
...
for i in range(Ncorridas):
    fid = open(f"caso_3_longdouble{i}.txt", "w")      # cambiar 
    dts = []  
    mem = []  
...
def plotting(Ncorridas): 
    ...
    data = sp.loadtxt("caso_3_longdouble0.txt")     #cambiar
    ...
    for i in range (1, Ncorridas):
        data = sp.loadtxt(f"caso_3_longdouble{i}.txt")  #cambiar
    ...
    plt.title("Rendimiento A Inv - Caso 3 - LongDouble")     #cambiar
    plt.grid(True)
    plt.xticks(xtcks, [])
