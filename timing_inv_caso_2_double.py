# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 00:17:26 2020
...
for i in range(Ncorridas):
    fid = open(f"caso_2_double{i}.txt", "w")      # cambiar 
    dts = []  
    mem = []  
...
def plotting(Ncorridas): 
    ...
    data = sp.loadtxt("caso_2_double0.txt")     #cambiar
    ...
    for i in range (1, Ncorridas):
        data = sp.loadtxt(f"caso_2_double{i}.txt")  #cambiar
    ...
    plt.title("Rendimiento A Inv - Caso 2 - Double")     #cambiar
    plt.grid(True)
    plt.xticks(xtcks, [])
