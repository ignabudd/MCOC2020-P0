# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 00:10:43 2020

@author: ignab
"""

from scipy import zeros
from numpy import single, fill_diagonal, matrix
...
for i in range(Ncorridas):
    fid = open(f"caso_1_single{i}.txt", "w")        #cambiar
    dts = []  
    mem = []  
...
def plotting(Ncorridas): 
    ...
    data = sp.loadtxt("caso_1_single0.txt")     #cambiar
    ...
    for i in range (1, Ncorridas):
        data = sp.loadtxt(f"caso_1_single{i}.txt")  #cambiar
    ...
    plt.title("Rendimiento A Inv - Caso 1 - Single")     #cambiar
    plt.grid(True)
    plt.xticks(xtcks, [])
