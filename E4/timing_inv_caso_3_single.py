# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 00:20:26 2020
...
from numpy import single, fill_diagonal, matrix    #ojo con estos
...
for i in range(Ncorridas):
    fid = open(f"caso_3_single{i}.txt", "w")      # cambiar 
    dts = []  
    mem = []  
    ...
    for N in Ns:
        ...
        dt = t2 - t1
        size = (N**2) * 4           # single (4 Bytes - 32 bits), varía también
...
def plotting(Ncorridas): 
    ...
    data = sp.loadtxt("caso_3_single0.txt")     #cambiar
    ...
    for i in range (1, Ncorridas):
        data = sp.loadtxt(f"caso_3_single{i}.txt")  #cambiar
    ...
    plt.title("Rendimiento A Inv - Caso 3 - Single")     #cambiar
    plt.grid(True)
    plt.xticks(xtcks, [])
