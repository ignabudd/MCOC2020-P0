# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 00:20:14 2020

@author: ignab
"""
from scipy import zeros, linalg
from numpy import double, fill_diagonal, matrix    #ojo con estos
...
for i in range(Ncorridas):
    fid = open(f"caso_3_double{i}.txt", "w")      # cambiar 
    dts = []  
    mem = []  
...
def plotting(Ncorridas): 
    ...
    data = sp.loadtxt("caso_3_double0.txt")     #cambiar
    ...
    for i in range (1, Ncorridas):
        data = sp.loadtxt(f"caso_3_double{i}.txt")  #cambiar
    ...
    plt.title("Rendimiento A Inv - Caso 3 - Double")     #cambiar
    plt.grid(True)
    plt.xticks(xtcks, [])
