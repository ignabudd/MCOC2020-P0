# -*- coding: utf-8 -*-
"""
Created on Wed Aug 12 00:19:34 2020

@author: ignab
"""

from scipy import zeros, linalg
from numpy import half, fill_diagonal, matrix    #ojo con estos
...
for i in range(Ncorridas):
    fid = open(f"caso_2_half{i}.txt", "w")      # cambiar 
    dts = []  
    mem = []  
...
def plotting(Ncorridas): 
    ...
    data = sp.loadtxt("caso_2_half0.txt")     #cambiar
    ...
    for i in range (1, Ncorridas):
        data = sp.loadtxt(f"caso_2_half{i}.txt")  #cambiar
    ...
    plt.title("Rendimiento A Inv - Caso 2 - Half")     #cambiar
    plt.grid(True)
    plt.xticks(xtcks, [])
